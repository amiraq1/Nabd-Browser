...
class PasswordStorage {
    async saveLogin(login: Omit<SavedLogin, "id" | "createdAt">): Promise<SavedLogin> {
        const currentState = await this.getAllLogins();

        // التحقق مما إذا كان موجوداً بالفعل وتحديثه
        const existingIndex = currentState.findIndex(l =>
            l.url === login.url && l.username === login.username
        );
        ...
        if (existingIndex >= 0) {
            currentState[existingIndex] = { ...currentState[existingIndex], ...login };
        } else {
